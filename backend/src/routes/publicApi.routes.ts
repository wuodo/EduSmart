...
// Debug: get or generate a unique API key for a tenant
router.get('/key', async (req, res) => {
  try {
    const { tenant: tenantSlug } = req.query as { tenant?: string };
    let tenant;
    if (tenantSlug) {
      const asId = parseInt(tenantSlug, 10);
      if (!isNaN(asId)) {
    ...
    // Always generate a unique key (replaces old one if exists)
    const apiKey = `edusmart-${tenant.id}-${Math.random().toString(36).slice(2, 10)}-${Date.now().toString(36)}`;
...
router.post('/inquiry', async (req, res) => {
  try {
    ...
    // Notify staff about new website inquiry
    try {
      ...
      for (const user of staffToNotify) {
        ...
        notifyStaff({
          ...
          title: inquiry.assignedTo ? 'New Inquiry Assigned' : 'New Website Inquiry',
          body: `"${inquiry.fullName}" (#${inquiry.id}) from website. Score: ${inquiry.score}/100.${inquiry.assignedTo ? '' : ' Unassigned — please assign.'}`,
